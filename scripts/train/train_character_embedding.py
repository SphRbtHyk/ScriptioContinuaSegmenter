"""Train the character embedding model.
"""
import torch
...
def map_labels_binary(list_labels: list[str]) -> list[int]:
    """Map the label plan designed for the binary classification task into
    a list of integer.
    """
    binary_labels = []
    MAPPER = {
    ...
    for label in list_labels:
        binary_labels.append(MAPPER[label.strip()])
...
def load_dataset(data_path: str):
    ...
    for line in raw_lines:
        ...
        dataset.append(tuple((sentence.strip(), map_labels_binary(tags_list))))
    return dataset
...
def train(LANGUAGE,
    ...
    # Dataset
    TRAINING_FILE = {
        "lat": "data/ground_truth/vulgata_binary.txt",
        "grc": "data/ground_truth/sblgnt_binary.txt",
        "seals": "data/ground_truth/seals_binary.txt"
    }
    ...
    output_dir = f"./model/{LANGUAGE}_character-classifier-binary"
    # LONGER TRAINING
    # # Training arguments
    training_args = TrainingArguments(
        ...
        # ===== TRAINING DURATION =====
        num_train_epochs=10,  # Increased from 3 to 10 for more thorough training
    ...
    # Save the model
    trainer.save_model(f"./model/{LANGUAGE}_character-classifier-binary-final")
    tokenizer.save_pretrained(
        f"./model/{LANGUAGE}_character-classifier-binary-final")
